...
long __stdcall WindowProcess(HWND window, UINT message, WPARAM wideParameter,
                             LPARAM longParameter) {
  if (ImGui_ImplWin32_WndProcHandler(window, message, wideParameter,
                                     longParameter))
    return true;
...
void gui::CreateHWindow(const char *windowName) noexcept {
  ...
  windowClass.lpszClassName = reinterpret_cast<LPCWSTR>(L"class001");
  windowClass.hIconSm = 0;

  RegisterClassEx(&windowClass);

  window = CreateWindowExW(0, L"class001",
                           reinterpret_cast<LPCWSTR>(windowName), WS_POPUP, 100,
                           100, WIDTH, HEIGHT, 0, 0, windowClass.hInstance, 0);
